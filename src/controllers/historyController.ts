...
const { ref_stage,entity_stage,entity_entity, entity, ref_entity_type,entity_attr_log,ref_attr,entity_attr } = modelsPls;
...
export const getHistory = async (req: Request, res: Response) => {
  try {
    ...
    const currentEntity = await entity.findOne({
      where: { entity_id: id },
      order: [['entity_attr_logs', 'rattr_id', 'ASC'], ['entity_attr_logs', 'ts_change', 'DESC']],
      include: [
        {
        ...
        },
        {
          ...
          as: 'entity_attr_logs',
          required: false,
          attributes: ['rattr_id', 'entity_attr_value', 'ts_change', 'user_change'],
          include: [
            {
              model: ref_attr,
              as: 'rattr',
    ...
    if (entity_entits) {
      ...
      if (rez.length > 0) {
        const [results, metadata] = await sequelize.query(`SELECT evl.entity_id , evl.attr_value_txt  from pls.entity_view_list evl where evl.entity_id in (${rez?.map(item => item.entity_id_link).join(',')})`);
        ...
        rez.forEach(item => {
          ...
          const foundItem = typedResults.find((result: any) => result?.entity_id == entity_id_link);
      ...
      for (const rez of entity_attr_logs) {
        if (rez.entity_attr_value != null) {
          if (currentRattrID != rez.rattr_id) {
            ...
            let result = entity_attrs?.filter((item) => item.rattr_id == currentRattrID);
    ...
    res.status(200).json({ "entity_id": currentEntity?.entity_id, "history": eventList.toJSON()});
  } catch (error) {
  console.error(error);
