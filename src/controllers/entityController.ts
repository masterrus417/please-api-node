...
const { entity,ref_entity_type, ref_attr, ref_attr_group, entity_attr, entity_stage, ref_stage, ref_route } = modelsPls;
  ...
  //получаем список сущностей
    ...
    export const createEntity = async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        const currentRout = await ref_route.findOne({ where: { rroute_id: currentType?.rroute_id } });
